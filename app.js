...
function cipher() {
	...
	if (!isNaN(textOriginal)) {//primero verifica si se ingresa numero o si la variable esta vacia
		...
		for (var j = 0; j < textOriginal.length; j++) {//itera letra por letra la frase
			var numeroAcsiiMin = textOriginal.charCodeAt([i]);//muestra a cada letra su valor en ascii
			var resultMin = (numeroAcsiiMin - 97 + 33) % 26 + 97;//ejecuta la formula del codigo cesar
			var textoCifradoMin = String.fromCharCode(resultMin);//el valor ascii lo cambia a letra

			letter += textoCifradoMin.toLowerCase();//cada letra la va concatenando a la nueva variable
		}
		document.write('Su palabra cifrada es: <strong>' + letter + '</strong>');//muestra la frase cifrada
	}
}
...
function decipher() {
	...
	if (!isNaN(textCifrado) || textCifrado.length < 0) {//primero verifica si se ingresa numero o si la variable esta vacia
		...
		for (var j = 0; j < textCifrado.length; j++) {//itera letra por letra la frase
			...
			var numeroAcsiiMin = textCifrado.charCodeAt([i]);//muestra a cada letra su valor en ascii
			var resultMin = (numeroAcsiiMin + 85 - 33) % 26 + 97;//ejecuta la formula del codigo cesar
			var textoCifradoMin = String.fromCharCode(resultMin);//el valor ascii lo cambia a letra
